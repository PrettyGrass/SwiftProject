...
class DTMethodController: DTBaseViewController {
 ...
 """)
        AdapterCellUtil.creatCell(section: section, text: "实例方法在类,结构体,枚举中的使用", needSeparator: true) { (cell, index) in
            //实例方法
            class A {
        ...
        AdapterCellUtil.creatCell(section: section, text: "类型方法在类,结构体,枚举中的使用", needSeparator: false) { (cell, index) in
            class class1 {
        ...
        AdapterCellUtil.descCell(section: section, desc: """
func 关键字之前加上关键字 static，来指定类型方法。类还可以用关键字 class 来允许子类重写父类的方法实现。
...
""")
        ...
        AdapterCellUtil.descCell(section: section, desc: """
 OC 里只有对象(实例)方法,还有类方法 ,Swift里叫实例方法,跟类属性方法 ,枚举 跟结构体内声明的一些方法,可以解决一些有关于结构体的一些 修改,或者是枚举有关的转化等业务,终于有地方可以写了,结构体提供拓展功能也是很方便,总的来说,结构体跟枚举使用场景更多了,以后各个类型的功能划分的更明确了
""")

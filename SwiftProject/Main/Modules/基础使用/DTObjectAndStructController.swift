//
//  ObjectAndStructController.swift
//  SwiftProject
//
...
class DTObjectAndStructController: DTBaseViewController {
    ...
    override func _demoCode() {
        ...
        AdapterCellUtil.creatCell(section: section, text: "类与结构体的对比", needSeparator: false) { (cell, index) in
...
4.定义构造器用于生成初始化值
5.可以拓展功能
6.遵循协议->可以对特定类型提供标准功能。
""")
        
        AdapterCellUtil.creatCell(section: section, text: "如何选择类与结构体", needSeparator: false) { (cell, index) in
...
tip:Swift 中，许多基本类型，诸如 String，Array 和 Dictionary,Set 类型均以结构体的形式实现。这意味着被赋值给新的常量或变量，或者被传入函数或方法中时，它们的值会被拷贝。
    Objective-C 中 NSString，NSArray 和 NSDictionary 类型均以类的形式实现，而并非结构体。它们在被赋值或者被传入函数或方法时，不会发生值拷贝，而是传递现有实例的引用。
""")
        
        AdapterCellUtil.creatCell(section: section, text: "类,结构体的定义与初始化", needSeparator: true) { (cell, index) in
            //实例
            struct PGSise {
        ...
        AdapterCellUtil.creatCell(section: section, text: "判断两个类是否相等", needSeparator: true) { (cell, index) in

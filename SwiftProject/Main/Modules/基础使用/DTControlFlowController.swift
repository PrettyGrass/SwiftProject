...
class DTControlFlowController: DTBaseViewController {
    ...
    override func _demoCode() {
        ...
        AdapterCellUtil.creatCell(section: section, text: "For-In的使用", needSeparator: true) { (cell, index) in
             //访问数组
        ...
        AdapterCellUtil.creatCell(section: section, text: "While 循环的使用", needSeparator: true) { (cell, index) in
//条件真 退出循环
//            while condition {
//                statements
//            }
...
//            } while 
        ...
        AdapterCellUtil.creatCell(section: section, text: "条件语句", needSeparator: true) { (cell, index) in
            //if
            if 1 < 3 {
                print("true")
            } else {
        ...
        AdapterCellUtil.creatCell(section: section, text: "控制转移语句", needSeparator: false) { (cell, index) in
...
throw: 表示抛出异常
""")
        AdapterCellUtil.creatCell(section: section, text: "带标签语句", needSeparator: false) { (cell, index) in
            let a = 3 ,b = 4
            myWhile: while a > b {
                break myWhile
            }
    ...
    AdapterCellUtil.descCell(section: section, desc: """
你可以使用标签（statement label）来标记一个循环体或者条件语句，对于一个条件语句，你可以使用 break 加标签的方式，来结束这个被标记的语句,continue 也是一样的
""")
        
        AdapterCellUtil.creatCell(section: section, text: "提前退出", needSeparator: false) { (cell, index) in
...
""")
        ...
        AdapterCellUtil.creatCell(section: section, text: "检测 API 可用性", needSeparator: false) { (cell, index) in
